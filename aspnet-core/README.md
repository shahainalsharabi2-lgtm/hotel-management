...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Modiaf.Al.Arab.Hotel.HttpApi.Host` and `Modiaf.Al.Arab.Hotel.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 637613d7-7eeb-410c-b949-f491f98433a9
```

> `637613d7-7eeb-410c-b949-f491f98433a9` is the password of the certificate, you can change it to any password you want.
...
Run `Modiaf.Al.Arab.Hotel.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Modiaf.Al.Arab.Hotel.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Modiaf.Al.Arab.Hotel.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
